...
The Kiro portal (app.kiro.dev) and Kiro management console only exist in `eu-central-1` (Frankfurt) and `us-east-1` (N. Virginia). If your IAM Identity Center instance is in a different region (we had it in `eu-west-1`), the portal cannot validate the IDC login and fails silently with a flickering error.
