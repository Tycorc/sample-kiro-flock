...
WAF_IP_SET_ID="$(get_output WafIpSetId)"

[[ -n "$BUCKET" && "$BUCKET" != "None" ]] || fail "Could not read BucketName output"

ok "Bucket   : $BUCKET"
ok "API URL  : $API_URL"
ok "WAF IPSet: $WAF_IP_SET_ID"
...
aws cognito-idp update-user-pool-client \
  ...
  --callback-urls "${API_URL}" \
  --logout-urls "${API_URL}" \
  --allowed-o-auth-flows implicit \
  --allowed-o-auth-scopes openid \
  --allowed-o-auth-flows-user-pool-client \
  --region "$REGION" >/dev/null
ok "Callback URL updated"
...
# Write auth config to S3 so the dashboard JS can pick it up
...
# ---------- 6. WAF IP allowlist ----------------------------------------------
step "6. WAF IP allowlist"

info "Updating WAF IP set to current public IP"
bash "${REPO_ROOT}/scripts/update-ip.sh" "$WAF_IP_SET_ID" "$REGION"

# ---------- Done -------------------------------------------------------------
step "Done"
ok "Dashboard: $API_URL"
ok "Bucket   : s3://$BUCKET"
ok "Next     : open the dashboard and click Start to launch the cluster"
